...
function cache:_CreateAdornment(Color3: Color3 | nil): AdornmentData
	local line: LineHandleAdornment = Instance.new("LineHandleAdornment")
	line.Name = DEFAULT_DEBUGGER_RAY_NAME
	line.Color3 = Color3 or DEFAULT_DEBUGGER_RAY_COLOUR
...
function cache:GetAdornment(Color3:Color3 | nil): AdornmentData?
	if #cache._AdornmentInReserve <= 0 then
		--- Create a new LineAdornmentHandle if none are in reserve
		local adornment: AdornmentData = cache:_CreateAdornment(Color3)
