...
function Utility:CreateBezierFunc(
	ControlPoints: { Vector3 },
	LUT_Size,
	FirstControlPointIndex: number,
	LastControlPointIndex: number
): BezierTable
	local n = (LastControlPointIndex and FirstControlPointIndex and (LastControlPointIndex - FirstControlPointIndex))
		or (#ControlPoints - 1)
	...
	local Bezier: RealBezierFunc = function(t: number)
		...
		if t == 0 then
			return ControlPoints[startIndex]
		elseif t == 1 then
			return ControlPoints[endIndex]
		end
		...
		for i = 0, n do
			...
			if not ControlPoints[controlIndex] then
				continue
			end
	...
	local LUT, AccumulatedLength =
		CreateLookUpTableForBezier(Bezier, LUT_Size or Utility.LUT_Size, FirstControlPointIndex or 1)
	...
	return {
		["Bezier"] = function(distance: number): Vector3
			local calculatedPos = Bezier(DistanceTo_time(LUT, distance, AccumulatedLength))

			if not isVectorValid(calculatedPos) then
				return savedSelf.Position
			else
				savedSelf.Position = calculatedPos
		...
		["Gradient"] = function(distance: number)
			...
			if not isVectorValid(calculatedGradient) then
				return savedSelf.Gradient
			else
				savedSelf.Gradient = calculatedGradient
				return calculatedGradient
			end
		end,
		["BezierID"] = ID,
...
function Utility:CreateCompositeBezier(Beziers: { BezierTable }): CompositeBezierTable
	...
	return {
		["CompositeBezier"] = function(distance: number): Vector3
			distance %= AccumulatedLength
		...
		["CompositeGradient"] = function(distance: number): Vector3
			distance %= AccumulatedLength
