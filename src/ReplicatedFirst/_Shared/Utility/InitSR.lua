...
local function InitHookedService_Receiver(LiveMockTestsAllowed, ModuleScript)
	...
	if ModuleScript:IsA("ModuleScript") and not (string.match(ModuleScript.Name, "^__Template")) then
		...
		if RequiredModule.new and RequiredModule.Run and not RequiredModule.Tag then -- has a life cycle, and doesn't have a tag for components
			local MockTestMs = if ModuleScript:FindFirstChild("MockTests")
				then require(ModuleScript:WaitForChild("MockTests"))
				else nil
					...
					then
						...
						if MockTestMs.SetupTestConfig then
							...
							if not MockTestingConfig then
								print(
									`Mock tests should return their config? | Take off the testing flag to silence. @{ModuleScript}`
								)
							end
						end
					end
			...
			if not (RunService:IsClient()) then
				...
				end -- some services use alot of mem, so they may need this so shutdown times are lower
			end
		end
	end
	--end)()
