...
function CheckIfResyncIsAllowed(self, isCheckingResync, accumulated)
	if isCheckingResync then
		return
	end -- so multiple resyncs don't run at a time
...
function UpdateTimeSinceLastTick(self)
	...
	if self.Connections["TimeSinceLastTick"] then
		self.Connections["TimeSinceLastTick"]:Disconnect()
	end
...
function UpdateServerTick(self, IsCheckingResync, dt)
	...
	while self["States"]._TimeAccumulated >= self.Settings.TickDelta do -- if its larger than the tD, it would run again
		...
		if self.Settings.TickDelta < self["States"]._TimeAccumulated - (self.Settings.TickDelta * 0.85) then
...
function Service:ArtificialAdvance(ResetAccumulatedTime: boolean, Time: number, Ticks: number, callback: () -> any)
	...
	if Time and Ticks then
		print("Make one of these variables nil please")
		return
	end
	if ResetAccumulatedTime then
		self._TimeAccumulated = 0
	end
